...
export async function crearSolicitudAction(formData: FormData) {
  ...
  const id = insert("solicitudes_compra", {
  ...
  audit({ usuario_id: user.id, accion: "crear", entidad: "solicitudes_compra", entidad_id: id });
...
export async function agregarPresupuestoAction(formData: FormData) {
  ...
  if (!proveedorId && nuevoProveedor) {
    proveedorId = insert("proveedores", { nombre: nuevoProveedor });
  }
  if (!proveedorId) throw new Error("Falta elegir o crear un proveedor.");

  insert("presupuestos_proveedor", {
  ...
  update("solicitudes_compra", solicitudId, { estado: "en_comparacion" });
...
export async function decidirCompraAction(formData: FormData) {
  ...
  const presupuesto = get<any>(`SELECT * FROM presupuestos_proveedor WHERE id = ?`, [presupuestoId]);

  const decId = insert("decisiones_compra", {
    solicitud_id: solicitudId, presupuesto_id: presupuestoId, decidido_por_id: user.id,
    motivo, monto: presupuesto?.precio ?? null,
  });
  update("solicitudes_compra", solicitudId, { estado: "aprobada" });
  audit({ usuario_id: user.id, accion: "aprobar_compra", entidad: "solicitudes_compra", entidad_id: solicitudId, valor_nuevo: { presupuestoId, motivo, monto: presupuesto?.precio } });
  revalidatePath(`/compras/${solicitudId}`);
  revalidatePath("/compras");
}
...
export async function marcarEntregadaAction(formData: FormData) {
  ...
  update("solicitudes_compra", id, { estado: "entregada" });
  revalidatePath("/compras");
